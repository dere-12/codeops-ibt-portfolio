...
function render() {
  ...
  if (filteredDishes.length === 0) {
    ...
    menuContainer.innerHTML = filteredDishes
      .map(
        (dish) => `
...
function validateOrderData({ name, phone }) {
  ...
  if (state.cart.length === 0)
    return "Your cart is empty. Add dishes before ordering.";

  return "";
}

function placeOrder(orderData) {
  const totalETB = state.cart.reduce(
    (sum, item) => sum + item.price * item.quantity,
    0,
  );
...
categoryNav.addEventListener("click", (e) => {
  ...
  document.querySelectorAll(".filter-btn").forEach((btn) => {
    btn.className =
      "filter-btn bg-gray-200 text-gray-700 px-4 py-1.5 rounded-full text-sm font-medium hover:bg-gray-300";
  });
  button.className =
    "filter-btn bg-emerald-700 text-white px-4 py-1.5 rounded-full text-sm font-medium";
